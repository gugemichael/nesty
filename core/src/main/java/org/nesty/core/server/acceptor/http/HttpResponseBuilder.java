...
public class HttpResponseBuilder {
    ...
    public static DefaultFullHttpResponse create(RequestContext requestContext, HttpResponseStatus status) {
        return create(requestContext, status, Unpooled.wrappedBuffer(uselessBuffer));
    }

    public static DefaultFullHttpResponse create(RequestContext requestContext, ByteBuf content) {
        return create(requestContext, HttpResponseStatus.OK, content);
    }

    static DefaultFullHttpResponse create(RequestContext requestContext, HttpResponseStatus status, ByteBuf content) {
        ...
        // fill nesty headers
        resp.headers().set(HttpConstants.HEADER_REQUEST_ID, requestContext.getRequestId());
